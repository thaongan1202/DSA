...
/**
 * QuickSortVisualizerPro
 * -------------------------------------------------
 * - Uses QuickSortInt variant (Hoare + median-of-three + cut-off to insertion sort)
 * - Bar chart with numeric values on bars
 * - Legend explaining colors
 * - Right-side panel to show step-by-step logs
 * - Controls: Start/Pause, Shuffle, Reset, Size, Speed (ms)
 *
 * Build & Run (JDK 17+):
 *   javac QuickSortVisualizerPro.java
 *   java QuickSortVisualizerPro
 */
public class QuickSortVisualizerPro extends JFrame {
    ...
    public QuickSortVisualizerPro() {
        super("QuickSort Visualizer Pro – Java Swing (QuickSortInt)");
    ...
    private JPanel buildLegendPanel() {
        ...
        legend.add(new JLabel("Value printed on each bar"));
        return legend;
    }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(QuickSortVisualizerPro::new);
    }
    ...
    static class VisualPanel extends JPanel implements Runnable {
        ...
        public void run() {
            try {
                ...
                SwingUtilities.invokeLater(() -> {
                    ...
                    if (w instanceof QuickSortVisualizerPro frame) {
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (int idx = 0; idx < n; idx++) {
                int val = a[idx];
                int barH = (int) Math.round(((double) val / n) * (h - 40));
                ...
                int th = fm.getAscent();
                int tx = x + (bw - tw) / 2;
                int ty = y - 2;                  // above bar by default
                if (barH > th + 6) {             // if tall enough, draw inside bar
                    ty = y + barH - 6;
            ...
            // Legend text line
            g2.setColor(Color.white);
            g2.drawString("Pivot: orange | i: blue | j: magenta | swap: red | bounds: cyan", 10, 18);
